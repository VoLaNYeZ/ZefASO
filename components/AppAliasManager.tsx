    ...
    return (
        <div className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <div className="space-y-3">
                <div className="grid grid-cols-[1.8fr_1fr_1fr_1fr] items-center text-[11px] uppercase font-semibold text-slate-500 dark:text-slate-400 leading-none px-2 text-center gap-2">
                    <span className="justify-self-start text-left">App ID</span>
                    <span className="justify-self-center">Abbr</span>
                    <span className="justify-self-center">{t.aliasNumber}</span>
                    <span className="justify-self-center">{t.aliasPrimary}</span>
                </div>

                {rows.map(row => (
                    <div key={row.appId} className="rounded-lg bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 px-3 py-1">
                        <div className="grid grid-cols-[1.8fr_1fr_1fr_1fr] items-center gap-3 text-center">
                            <span className="text-sm font-medium text-slate-800 dark:text-slate-100 truncate text-left justify-self-start max-w-full leading-tight">{idLabelMap?.[row.appId] || `${appName} (${row.appId})`}</span>
                            ...
                            <div className="flex justify-center justify-self-center">
                                <input
                                    ...
                                    className="w-12 px-2 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm font-semibold text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 outline-none text-center"
                                />
                            </div>

                            <div className="flex justify-center justify-self-center pl-1">
                                <input
                                    value={row.number}
                                    ...
                                    maxLength={4}
                                    placeholder="#"
                                    className="w-14 px-2 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm font-semibold text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 outline-none text-center"
                                />
                            </div>
